...
local stats_text = wibox.widget({
	...
	markup = helpers.ui.colorize_text("Stats", "#786A62"),
	valign = "center",
...
local function cpu()
	...
	watch(
		...
		function(_, stdout)
			...
			tooltip:set_markup_silently(helpers.ui.colorize_text(cpu_value .. "%", "#FFD9C2"))
	...
	do
		...
		function(stdout)
			...
			sh -c "cat ]] .. temp_path .. [["
				...
				tooltip:set_markup_silently(helpers.ui.colorize_text(temp_value .. "°C", "#FDC4A1"))
				collectgarbage("collect")
			end)
		end
	)
...
local function ram()
	...
	watch(
		...
		function(_, stdout)
			...
			tooltip:set_markup_silently(helpers.ui.colorize_text(string.format("%.1f", used / 1000) .. "G", "#E4A984"))
			collectgarbage("collect")
		end
	)
...
local function hdd()
	...
	watch([[bash -c "df -h /home|grep '^/' | awk '{print $5}'"]], 180, function(_, stdout)
		...
		tooltip:set_markup_silently(helpers.ui.colorize_text(space_consumed .. "%", "#C68A65"))
		collectgarbage("collect")
	end)
